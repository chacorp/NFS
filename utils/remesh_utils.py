...
class ICT_face_model():
    def __init__(self, face_only=True, scale=0.1, base_dir=None, use_decimate=False, device='cpu'):
        ...
        if face_only:
            ## face only
            v_idx, quad_f_idx = 9409, 9230
        else:
            ## face + head + neck
            v_idx, quad_f_idx = 11248, 11144
    ...
    def get_mesh(self, mesh_std=False):
        """
        Args:
            mesh_std (bool): if True, apply `mesh standardization`
        """
        mesh = trimesh.Trimesh(vertices=self.neutral_verts, faces=self.faces)
    ...
    def apply_coeffs(self, id_coeff, exp_coeffs=None, mesh_v_idx=None, return_all=False):
        """
        Args:
    ...
    def apply_coeffs_batch(self, id_coeff, exp_coeffs=None, mesh_v_idx=None, return_all=False):
        """
        Args:
        ...
        id_verts = self.neutral_verts[:, :self.v_idx] + id_disps
    ...
    def apply_coeffs_batch_torch(self, id_coeff, exp_coeffs=None, mesh_v_idx=None, return_all=False):
        """
        Args:
        ...
        exp_disps = exp_disps[:, :self.v_idx]
            
        id_verts = torch.from_numpy(self.neutral_verts[:, :self.v_idx]).float().to(device) + id_disps
        id_exp_verts = id_verts + exp_disps
    ...
    def get_id_disp(self, id_coeff=None, mesh_v_idx=None):
        """
        Args:
    ...
    def get_exp_disp(self, exp_coeffs=None, mesh_v_idx=None):
        """
        Args:
            exp_coeffs (np.ndarray): [T, 53] ICT-facekit expression coeff
            mesh_v_idx (np.ndarray): <int> array of std mesh vertex indices
        ...
        if exp_coeffs is not None:
            ...
            T = exp_coeffs.shape[0]
            #exp_disps = np.einsum('jk,kls->jls', exp_coeffs.float(), self.exp_basis.to(device))[:, :self.v_idx]
...
if __name__ == '__main__':
    ...
    magnify = 10 
    original_vertices   = magnify * ict_mesh.vertices
    subdivided_vertices = magnify * ict_mesh_decimate.vertices
        
    
    selected_vertex_indices = compute_origin_vertex_info(original_vertices, subdivided_vertices)
    print(selected_vertex_indices)
    
#     recon_decimated = trimesh.Trimesh(
#         vertices=ict_mesh.vertices[selected_vertex_indices], 
#         faces=ict_mesh_decimate.faces
#     )
    
    recon_decimated_dict = {
        'unsub_v_idx': selected_vertex_indices, 
        'unsub_f_idx': ict_mesh_decimate.faces
    }
#     np.save('tmp/ict-unsub_info.npy', recon_decimated_dict)
#     ssssss = np.load('tmp/ict-unsub_info.npy', allow_pickle=True).item()
#     unsubd_vertex_indices = ssssss['unsub_v_idx']
#     unsubd_faces_indices  = ssssss['unsub_f_idx']
    
    vertices_A = ict_mesh.vertices
    vertices_B = ict_mesh_subdivide[0]
    print(vertices_A.shape)
    print(vertices_B.shape)
    
    
    added_vertex_info = compute_added_vertex_info(vertices_A, vertices_B)
    reconstructed_vertices = reconstruct_mesh(vertices_A, added_vertex_info)
    
    print(reconstructed_vertices.shape)
    
    
    t_subdivide_path      = '/source/sihun/MAASA/ICT/t_subdivide'
    t_unsubdivide_path    = '/source/sihun/MAASA/ICT/t_unsubdivide'
    os.makedirs(t_subdivide_path, exist_ok=True)
    os.makedirs(t_unsubdivide_path, exist_ok=True)

    for temp_coeffs_path in sorted(glob.glob(f'/source/sihun/MAASA/ICT/templates/*_coeffs.pt')):
        print(temp_coeffs_path)
        
        temp_coeffs_name = temp_coeffs_path.split('/')[-1].split('.')[0]
        
        temp_coeffs = torch.load(temp_coeffs_path)
        temp_disps  = torch.einsum('bi, ijk->bjk', temp_coeffs[None], ict_face_model.id_basis).squeeze(0)
        temp_verts = ict_face_model.neutral_verts[std_mesh_v_idx] + temp_disps[std_mesh_v_idx]
        
        reconstructed_vertices = reconstruct_mesh(temp_verts, added_vertex_info)

        recon_decimated = trimesh.Trimesh(
                vertices=temp_verts[selected_vertex_indices], 
                faces=unsubd_faces_indices
            )
        _ = recon_decimated.export(f'{t_unsubdivide_path}/{temp_coeffs_name}.obj')

        recon_subdivide = trimesh.Trimesh(
                vertices=reconstructed_vertices, 
                faces=ict_mesh_idx[1]
            )
        ## filter bumpy surface
        recon_subdivide = trimesh.smoothing.filter_laplacian(
            recon_subdivide, lamb=0.12, iterations=10, 
            implicit_time_integration=False, volume_constraint=True, laplacian_operator=None
        )
        _ = recon_subdivide.export(f'{t_subdivide_path}/{temp_coeffs_name}.obj')