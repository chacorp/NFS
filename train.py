...
class Trainer():
    ...
    def train_stage1(self, epochs):
        ...
        self.valid_dataset = NFSDataset(self.opts, is_valid=True)
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for index, batch in pbar:
                ...
                loss_dict, pred_vertices, _, pred_exp, pred_id, pred_seg = self.model(batch, return_all=True, stage=1, epoch=epoch)
                ...
                if train_counter % interv == 1:
                    ...
                    if pred_seg is not None:
                        ...
                        if mesh_data == 'ict':
                            c_list=[gt_seg, gt_seg, pred_seg, pred_seg]
                        else:
                            c_list=[pred_seg] * len_v
            ...
            # save model
            if epoch % 100 == 0:
                torch.save(self.model.state_dict(), f'{self.opts.log_dir}/wav2rig_{epoch:03d}.pth')
            ...
            for index, batch in tqdm(enumerate(self.valid_dataloader), total=len_valid_data, ncols=100):
                ...
                with torch.no_grad():
                    loss_dict, pred_vertices, _, pred_exp, pred_id, pred_seg = self.model(batch, return_all=True)
            ...
            if running_losses["total"]/counter < BEST_LOSS:
                ...
                torch.save(self.model.state_dict(), f'{self.opts.log_dir}/wav2rig_best.pth')
