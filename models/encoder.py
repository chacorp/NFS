...
class NewDiffusionNetEncoder(nn.Module):
    # reference: https://github.com/dafei-qin/NFR_pytorch/blob/e3553faa77f65240ec20167aec6e814473233890/mymodel.py#L17
    def __init__(self, opts, in_shape=6, out_shape=128, hid_shape=256, pre_computes=None, N_block=4, outputs_at='vertices', with_grad=True):
    ...
    def forward(self,
        ...
        outputs_mean = torch.sum(outputs * batch_mass.unsqueeze(-1), dim=-2) / torch.sum(batch_mass, dim=-1, keepdim=True)
        return outputs[..., :self.opts.seg_dim], outputs_mean[..., self.opts.seg_dim:]
    

class AllDiffusionNetEncoder(BaseDiffusionNetEncoder):
    # reference: https://github.com/dafei-qin/NFR_pytorch/blob/e3553faa77f65240ec20167aec6e814473233890/mymodel.py#L17
    def __init__(self, in_shape=6, out_shape=128, hid_shape=256, pre_computes=None, N_block=4, outputs_at='vertices', with_grad=True):
        super(AllDiffusionNetEncoder, self).__init__(in_shape, out_shape, hid_shape, pre_computes, N_block, outputs_at, with_grad)
    ...
    def forward(self,
        ...
        outputs_mean = torch.sum(outputs * batch_mass.unsqueeze(-1), dim=-2) / torch.sum(batch_mass, dim=-1, keepdim=True)
        # [seg_code, id_code, exp_code] == [20, 128, 128]
        return outputs[..., :self.opts.seg_dim], outputs_mean[..., self.opts.seg_dim:self.opts.seg_dim+128], outputs_mean[..., self.opts.seg_dim+128:]