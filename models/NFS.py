...
class NFS(nn.Module):
    def __init__(self, 
        ...
        self.ict_precompute_fo = f'{abs_path}/ICT/precompute-synth-face_only'
        self.ict_precompute = f'{abs_path}/ICT/precompute-synth-fullhead'
        #---------------------------------------------------------------------------------
        ...
        self.ict_vert_segment_fo = self.ict_vert_segment[:self.ict_face_model_fo.v_idx]
        #---------------------------------------------------------------------------------
    ...
    def get_img_feat_from_mesh(self, tgt_mesh, return_feat=False):
        ...
        if return_feat:
            tgt_img_feat = self.get_img_feat(tgt_img)[None] #------------------- [1, 1, 128]
            return tgt_img_feat
        else:
            return tgt_img
    ...
    def get_local_feature(self, vertices, faces, img_feat, at='verts'):
        ...
        if at=='verts':
            ...
            verts_img_feat = img_feat.repeat(B, V, 1) # [1, V, 128]
            ...
            tri_img_feat = img_feat.repeat(B, F, 1)
    ...
    def get_inputs_ict(self, pred_exp_coeff):
        ...
        """
        img_feat_ict = self.get_img_feat(self.neu_img) # [1, 1, 128]
    ...
    def encode_mesh(self,
        ...
        src_img_feat = self.get_img_feat(src_img)[None] #------------------- [1, 1, 128]
    ...
    def decode_grad(self, inputs, batch_process=True):
        ...
        ----------
            inputs:
                ...
                pred_id_coeff (torch.tensor): [1, ID]
                pred_seg_coeff (torch.tensor): [1, V, Seg]
        ...
        if batch_process:
            ...
            for pred_rig in pred_exp_coeff:
                if 'nfr' in self.design:
    ...
    def stage1_forward(self, batch, teacher_forcing=True, return_all=False, epoch=0):
        ...
        ----------
            batch:
                audio_feat (torch.Tensor):    [B, W, 768] not used here
                id_coeff (torch.Tensor):      [1, 128]    ICT-face model id coeff
                gt_rig_params (torch.Tensor): [B, W, 128] ICT-face model exp_coeff 
                ...
                vertices (torch.Tensor):      [B, W, V, 3] sequence of mesh vertices (animation)
                faces (torch.Tensor):         [B, F, 3] mesh faces indices
                img (torch.Tensor):           [B, 256, 256, 3] rendered mesh image
        ...
        gt_rig_params = batch.gt_rig_params.squeeze()
        template = batch.template
        gt_vertices = batch.vertices.squeeze()
        faces = batch.faces
        img = batch.img
        ...
        img_feat = self.get_img_feat(img)[None] # [1, 1, 128]
        ...
        pred_id_coeff, pred_exp_coeff, pred_seg_coeff = self.encode_mesh_grad(vert_feat, vert_feat_exp, batch.get_dfn_info)
        ##------------------------------------------------------------------------------
        ...
        if mesh_data == 'ict':
            loss["recon_rIEnc"] = self.criterion(gt_id_coeff, pred_id_coeff)
            loss["recon_rEEnc"] = self.criterion(gt_rig_params, pred_exp_coeff)
            ...
            if not self.opts.no_BP:
                ...
                loss["recon_vICT"] = self.criterion(gt_vertices, pred_ICT_recon)
            ...
            if not self.opts.no_BR:
                ...
                loss["recon_vICT"] = self.criterion(gt_recon_ICT, pred_outputs_ict[:,:ict_model.v_idx])

            if 'new2' in self.design:
                pred_seg_log = F.log_softmax(pred_seg_coeff, dim=-1).squeeze(0) ## [V, Seg]
                loss["nll_vSeg"] = F.nll_loss(pred_seg_log, ict_seg)
        else:
            loss["recon_rIEnc"] = self.non_ict_loss(pred_id_coeff[:, :100]) + self.L2_regularization(pred_id_coeff[:, 100:])
            loss["recon_rEEnc"] = self.non_ict_loss(pred_exp_coeff[:, :53]) + self.L2_regularization(pred_exp_coeff[:, 53:])
    ...
    def stage1_evaluate(self, batch, batch_process=False, return_all=False, epoch=0):
        ...
        gt_rig_params = batch.gt_rig_params.squeeze(0).to(self.device)
        template = batch.template.to(self.device)
        gt_vertices = batch.vertices.squeeze(0).to(self.device)
        ...
        with torch.no_grad():
            ...
            style_emb = None
            inputs = (local_feat, pred_exp_coeff, pred_id_coeff, pred_seg_coeff, style_emb, template, faces, operators)
            pred_outputs, pred_jacobians = self.decode(inputs, batch_process=False)
        torch.cuda.empty_cache()
        ##------------------------------------------------------------------------------
