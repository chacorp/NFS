...
def render_mesh_helper(\
    ...
    tri_mesh = trimesh.Trimesh(vertices=mesh_copy.v, faces=mesh_copy.f, vertex_colors=vertex_color)
...
def render_sequence(\
    ...
    fn="sample",\
    id="m00",\
    template_dir="./ICT/templates_std/{}_temp_transformed.obj"\
    ):
    ...
    template_file = template_dir.format(id)
    # import pdb; pdb.set_trace()
    print(template_file)
    ...
    for i_frame in tqdm(range(num_frames)):
        ...
        pred_img = render_mesh_helper(render_mesh, center, vertex_color=None, z_offset=-1.3)
...
def render_sequence_test(\
    verts, \
    output_path,\
    wav_path,\
    fn="sample",\
    template_file="./ICT/templates_std/{}_temp_transformed.obj"\
    ):
    os.makedirs(output_path, exist_ok=True)
    print("rendering sequence...")
    print(f"\t[{fn}] {wav_path}")
